...
// Test(request_line, valid_with_path) try {
// 	pid_t	server_pid;
// 	int	client_socket;
...
// 	// Note: In the future, replace this with a proper request
// 	std::string	request = "GET /note.txt HTTP/1.1\r\n";
// 	ssize_t		send_size;
...
// 	char		response[14];
// 	std::string	expected = std::string("HTTP/1.1 200 ");
// 	ssize_t		recv_size;

// 	// TODO: handler smaller packets
// 	if ((recv_size = recv(client_socket, response, expected.size(), 0)) != static_cast<ssize_t>(expected.size())) {
// 		close(client_socket);
// 		stop_server(server_pid);
// 		throw std::runtime_error(std::string("The client failed to retreive the response: ") + strerror(errno));
// 	}
...
// 	if (response != std::string("HTTP/1.1 200 "))
// 		throw std::runtime_error(std::string("Wrong status code, expected 200."));
// } catch (std::runtime_error e) {
// 	cr_assert(false, "Error: %s", e.what());
// }
...
Test(request_line, valid_with_wildcard) try {
	pid_t	server_pid;
	int	client_socket;
